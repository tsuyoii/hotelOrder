...
<script>
  export default{
    data () {
      return {
      //rules通常用于表单规则制定，required表示是否必须填写，当光标离开（不再聚焦于该栏时）触发规则提示，提示信息：“用户名不能为空”
        rules: {
          username: [{required: true, message: '用户名不能为空', trigger: 'blur'}],
          password: [{required: true, message: '密码不能为空', trigger: 'blur'}],
          responseResult:[]
    ...
    methods: {
      login () {
        ...
        this.$axios
          ...
          .then(successResponse => {
            this.responseResult = JSON.stringify(successResponse.data)//即将返回的JSON格式转换为字符串赋值给responseResult,
            if (successResponse .data.code === 200) {
              this.$router.replace({path:'/index'})
            } else {
              this.$alert(successResponse.data.message, '提示', {
